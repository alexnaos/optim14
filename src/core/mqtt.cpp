...
void mqttInit() {
  Serial.printf("##MQTT#:\tInit %s:%d user='%s' topic='%s'\n", MQTT_HOST, MQTT_PORT, MQTT_USER, MQTT_ROOT_TOPIC);
  ...
  Serial.println("##MQTT#:\tCalling connectToMqtt()");
  connectToMqtt();
}
...
void onMqttDisconnect(AsyncMqttClientDisconnectReason reason) {
  ...
  if (WiFi.isConnected()) {
    Serial.println("##MQTT#:\tWiFi OK, scheduling reconnect");
    xTimerStart(mqttReconnectTimer, 0);
  } else {
    Serial.println("##MQTT#:\tWiFi DOWN, waiting for reconnect");
  }
}
...
void mqttPublishStatus() {
  if(mqttClient.connected()){
    ...
    Serial.printf("##MQTT#:\tPublish status: topic=%s\n", topic);
    mqttClient.publish(topic, 0, true, status);
  } else {
    Serial.println("##MQTT#:\tPublish status SKIPPED - not connected");
  }
}
