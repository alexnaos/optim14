...
void Player::_stop(bool alreadyStopped){
  ...
  // setDefaults() удалён из публичного API новой версии библиотеки
...
void Player::initHeaders(const char *file) {
  ...
  connecttoFS(SD,file);
  // eofHeader удалён — крутим цикл пока играет
  for(int i=0; i<200 && isRunning(); i++) Audio::loop();
  // setDefaults() удалён из публичного API
}
