...
void ticks()
{
  ...
  if (player.isRunning() && config.store.play_mode == PM_SDCARD)
...
bool connectToWiFi()
{
  ...
  wifiMulti.addAP("Sloboda100", "2716192023"); // SSID and PW in code
  ...
  // Добавьте эти строки прямо перед wifiMulti.run();
  WiFi.disconnect(true);
  delay(100);
  WiFi.mode(WIFI_STA);
  WiFi.begin("Sloboda100", "2716192023"); // Прямой вызов часто надежнее Multi
...
void Network::begin()
{
  ...
  // 1. Очистка старого мусора и установка режима клиента
  WiFi.disconnect(true, true);
  delay(500);
  WiFi.mode(WIFI_STA);

  // 2. Прямая попытка подключения к вашей сети
  Serial.print("##[WIFI]# Connecting to Sloboda100");
  WiFi.begin("Sloboda100", "2716192023");

  // Ждем подключения максимум 15 секунд (30 попыток по 500мс)
  int retry = 0;
  ...
  {
    // Если не вышло — поднимаем точку доступа для настройки
    Serial.println("\n##[WIFI]# Failed to connect to Sloboda100. Raising AP...");
    raiseSoftAP();
    return; // Выходим, чтобы не выполнять код для CONNECTED
  }

  // 4. Инициализация остальных систем (только если CONNECTED)
  config.initNetwork();
  ...
  // Запуск таймера тиков и внешних коллбэков
  ctimer.attach(1, ticks);
  if (network_on_connect)
    network_on_connect();

  Serial.println("##[BOOT]#\tnetwork done");
}
...
bool getWeather(char *wstr)
{
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: description not found !");
    return false;
  }
  tmps += 15;
  tmpe = strstr(tmps, "\",\"");
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: icon not found !");
    return false;
  }
  tmps += 8;
  tmpe = strstr(tmps, "\"}");
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: temp not found !");
    return false;
  }
  tmps += 7;
  tmpe = strstr(tmps, ",\"");
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: feels_like not found !");
    return false;
  }
  tmps += 13;
  tmpe = strstr(tmps, ",\"");
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: pressure not found !");
    return false;
  }
  tmps += 11;
  tmpe = strstr(tmps, ",\"");
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: humidity not found !");
    return false;
  }
  tmps += 10;
  tmpe = strstr(tmps, ",\"");
  ...
  if (grnd_level_pr)
  {
    ...
    if (tmpe == NULL)
    {
      Serial.println("## OPENWEATHERMAP ###: grnd_level not found !");
      return false;
    }
    strlcpy(press, tmps, tmpe - tmps + 1);
    cursor = tmpe + 2;
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: wind speed not found !");
    return false;
  }
  tmps += 8;
  tmpe = strstr(tmps, ",\"");
  ...
  if (tmps == NULL)
  {
    Serial.println("## OPENWEATHERMAP ###: wind deg not found !");
    return false;
  }
  tmps += 6;
  tmpe = strstr(tmps, ",\"");
