...
void setup() {
  ...
  while(!display.ready()) delay(10);
    ...
    Serial.println("##MQTT#:\tMQTT_ROOT_TOPIC defined, calling mqttInit()");
    mqttInit();
  #else
    Serial.println("##MQTT#:\tMQTT_ROOT_TOPIC NOT defined, skipping MQTT");
  #endif
...
void checkConnection(){
  ...
  if ((WiFi.status() != WL_CONNECTED) && (millis() - checkMillis >=checkInterval)) {
    ...
    checkMillis = millis();
    #ifdef MQTT_ROOT_TOPIC
      Serial.println("##MQTT#:\tReconnecting MQTT after WiFi restore");
      connectToMqtt();
    #endif
  }
}
